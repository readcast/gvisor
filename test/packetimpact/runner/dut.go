...
func setUpDUT(ctx context.Context, t *testing.T, id int, mkDevice func(*dockerutil.Container) DUT) (dutInfo, error) {
	...
	if native {
		dut = dockerutil.MakeNativeContainer(ctx, logger(fmt.Sprintf("dut-%d", id)))
	} else {
		dut = dockerutil.MakeContainer(ctx, logger(fmt.Sprintf("dut-%d", id)))
	}
	info.container = dut
	...
	if _, err := mountTempDirectory(t, &runOpts, "dut-output", testOutputDir, true); err != nil {
		return dutInfo{}, err
	}

	device := mkDevice(dut)
	ipv4PrefixLength, _ := testNet.Subnet.Mask.Size()
	remoteIPv6, remoteMAC, dutDeviceID, dutTestNetDev, err := device.Prepare(ctx, t, runOpts, ctrlNet, testNet)
	if err != nil {
		return dutInfo{}, err
	}
...
func TestWithDUT(ctx context.Context, t *testing.T, mkDevice func(*dockerutil.Container) DUT) {
	...
	dockerNetworks := make([]*dockerutil.Network, 2*numDUTs)
	dutTestNets := make([]*testbench.DUTTestNet, numDUTs)
	dutContainers := make([]*dockerutil.Container, numDUTs)
	...
	for i := 0; i < numDUTs; i++ {
		...
		case info := <-dutInfoChan:
			dockerNetworks[2*i] = info.ctrlNet
			dockerNetworks[2*i+1] = info.testNet
			dutTestNets[i] = info.netInfo
			dutContainers[i] = info.container
		case err := <-errChan:
			t.Fatal(err)
		}
	}
	...
	if _, err := mountTempDirectory(t, &runOpts, "testbench-output", testOutputDir, true); err != nil {
		t.Fatal(err)
	}
	localDutTestNetsDir, err := mountTempDirectory(t, &runOpts, "dut-test-nets", containerDutTestNetsDir, false)
	if err != nil {
		t.Fatal(err)
	}
...
// getDevicesInfo will run "ip addr show" on the container and parse the output
...
// getOrAssignIPv6Addr will try to get the IPv6 address for the iterface; if an
...
// and then mounted into the container under the name provided. The temporary
// directory name is returned. Content in that directory will be copied to
// TEST_UNDECLARED_OUTPUTS_DIR if isOutput is true.
func mountTempDirectory(t *testing.T, runOpts *dockerutil.RunOpts, hostDirTemplate, containerDir string, isOutput bool) (string, error) {
	t.Helper()
